...
export const initSocket = (httpServer: any): Server => {
  ...
  io.on('connection', (socket: CustomSocket) => {
    ...
    // Handle new bid
    socket.on('place_bid', async (data: { auctionId: string, amount: string }, callback: (res: any) => void) => {
      ...
      try {
        ...
        const currentMax = Number(auction.currentHighestBid) || Number(auction.startingPrice);
        const minValid = currentMax + Number(auction.minIncrement);
